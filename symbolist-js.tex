...
Note that in \symbolist mapping between \textit{performance media} and \textit{graphic representation} is achieved through first mapping to semantic data.
...
\subsection{Data Format}\label{subsec:format}
...
% there are two top level symbols: top-svg and top-html, not sure if we need to say that here or not... probably not

\subsection{Score, File Format}\label{}
