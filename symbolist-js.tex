...
mention that in addition to the front end that the drawsocket message format
...
\subsubsection{Interprocess messaging syntax} 
...
\begin{lstlisting}[  language=Javascript, mathescape, columns=fullflexible, basicstyle=\oscfontsize\fontfamily{lmvtt}\selectfont ]
{
    /key : "data",
    /val : {
...
It could also be possible to attach links to other OSC sequencing applications like IRCAM's Antescofo expression language.
...
In the area of notation for spatial movement, there are plans to continue development of trajectories (visible in the curves attached to note events in Figure \ref{fig:screenshot}, and to connect \symbolist with the ICST's Spatialization Symbolic Music Notation (SSN).

In the audio domain, it could be interesting to develop tools for development of signal processing graphs that could be interpreted and performed in other applications, for example generating Faust\footnote{https://faust.grame.fr/} or Gen~ DSP code, SuperCollider instruments etc. %, or even potentially creating connections with other web-based visual programming interfaces like purr-data\footnote{https://git.purrdata.net/jwilkes/purr-data} and cables.gl\footnote{https://cables.gl/}.
...
A special thanks to James Tsz-Him Cheung for his work on the common practice notation definitions, which is greatly helping push \symbolist forward, and especially thank you to Prof. Georg Hajdu for his collaboration and ongoing support for this work.
