...
MVC~\footnote{https://developer.mozilla.org/en-US/docs/Glossary/MVC} %\cite{krasner1988description}
...
Loosely following the MVC pattern, the concerns of drawing are kept within the browser-side \uicontroller, with all messages between the \textit{ui} and \textit{io} processes are in \drawsocket JSON format, with all symbol data expressed in its \textit{semantic} form.
...
Since the ui and io symbol definitions run in parallel they need to keep the other side up to date in case of any alteration to the score data. 
