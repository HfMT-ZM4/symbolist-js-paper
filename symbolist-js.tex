...
\def\ioapiFunction{\textit{io\_ap}}
\def\ioapi{\textit{io\_ap}\xspace}
\makeatletter
...
Currently, definition scripts are programmed directly in JS -- eventually it is planned to provide a UI in the editor for defining a mapping definition graphically, but this is not yet implemented.
...
As mentioned above, the graphic representation takes advantage of the HTML dataset feature, which provides quick access to semantic data for ui-definitions, stored in the top level SVG $<$g$>$ or HTML $<$div$>$ elements. 
And so, since ui-definitions are running in a browser window, they are able to make use of the standard HTML DOM tree JS methods for fast querying of elements in the graphic version of the score\footnote{https://developer.mozilla.org/en-US/docs/Web/API/Document} (i.e. querySelector, getElementById, etc.).

To provide similar data query access on the server-side, the score data is stored in two JS objects, the \textit{score} which stores the semantic data in a hierarchical tree structure, and a JS Map object named \textit{model} which is a flat lookup table by unique id, with links to object references to the coordinated object in the score tree.

Since the ui- and io-definitions run in parallel they need to keep the other side updated in case of any alteration to the score data. 
...
\section{Symbol Definition Class Structure}
Symbols are defined as JS classes which are stored and recalled when symbol actions are to be performed. In each controller context (ui or io) there are certain required methods and variables that are expected in order for the class to function properly in the \symbolist ecosystem.
...
In the template class data to graphic mapping is handled in two internal methods:

viewParamsToData
dataToViewParams
\textit{childDataToViewParams} mapping function in container symbol called from children to request view parameters from data object.

\textit{childViewParamsToData} mapping function in container symbol called from children to get data mapping from view parameters.
...
\item \textit{getInfoDisplay}: called when creating the inspector window; returns drawing commands for the inspector contextual menu, for convenience there is a makeDefaultInfoDisplay in the \uiapi which can be used for most cases.
...
\section{IO Definitions}\label{subsec:io_messages}
